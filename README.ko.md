...
- **프로젝트 오버레이** — `<repo>/.outsource/overlay.md`. base 브랜치, 사내 게이트 레시피, 사고 이력 — 그 코드 옆에 있어야 하는 사실들.

둘 다 자동 적용되며 spec 조립에 사용자 → 프로젝트 순으로 포함합니다.

프로젝트 오버레이를 체크인하는 것이 기본값입니다. 다만 **한 레포의 체크아웃이 한 머신에 여러 개**일 때 — 클론 여러 벌 + 워크트리, 각자 다른 브랜치 — 기본값이 틀려집니다. 체크인된 오버레이는 브랜치 수만큼 갈라지는 사본이 되고, 리드가 그때 서 있던 클론에서 오버레이를 고치면 나머지 전부의 규칙이 조용히 포크됩니다. 실측: 체크아웃 16개인 레포에서 몇 달 간격으로 쓰인 오버레이 두 벌이 서로를 모른 채 공존했습니다. 파일 한 벌을 체크아웃 **밖**에 두고 각 체크아웃에 `.outsource` 심링크를 걸고, `.git/info/exclude` 로 체크아웃마다 무시하십시오 — 워크트리는 메인 체크아웃의 exclude 를 공유하므로 한 줄이면 전부 덮습니다. 오버레이 탐지는 심링크를 그대로 따라가므로 그 외에 바뀌는 것은 없습니다.
