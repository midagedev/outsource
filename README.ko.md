...
**4라운드, 과제 유형 둘을 바꿔, 두 팔 모두 `max`** — 지금까지는 커널 하나를 세 번 다듬은 시리즈였으므로 마지막 라운드는 과제 부류를 둘 바꿨습니다. CPU 커널(Rust AVX2 intrinsics로 `Q3_K×Q8_K` gemv, CCD를 아는 핀 고정 스레드, 317 MB 작업 집합에서 ik_llama.cpp CPU 백엔드와 대조; 게이트는 각 엔진 최적 스레드에서 ggml의 0.9× 이상)과 컴파일러 버그 추적(cuda-oxide `#[unroll]` ICE의 최소 재현기, 변수 하나씩 바꾸는 개입 표, 패스 위치, 중복 검색, 영문 이슈 초안 — GitHub 쓰기 금지). 네 팔, 네 워크트리, 게이트는 전부 리드가 재실행:

| 4라운드 (`max`) | glm-5.3 | muse-spark-1.3 |
|---|---|---|
| **CPU gemv** — 벽시계 | 34분 | **18분** |
| big M=1 대 ggml, 리드 재측정 | 1.11× | 1.11× |
| big M=8 | **0.87×** | 0.57× |
| expert0 M=1 (작은 형상) | **0.91×** | 0.12× (스레드당 22행 미만에서 배리어 잡음) |
| 64스레드(SMT), big M=1 | **유지** (−0.1 %) | −12 % |
| 팔이 찾은 것 | 자기 포트의 스케일 언팩 버그를 ggml 디퀀트 덤프와 대조해 잡음 | 레버 표: THP 미발동, 2배 언롤 −25 %, SW 프리페치 −10 % — 전부 증거와 함께 되돌림 |
| 채택 | **예** (커널) | 레버 표를 노트에 편입 |
| 토큰(출력, 추론 포함 / 턴) | 456k / 160 | **77k + 추론 48k / 52** |
| **ICE 추적** — 벽시계 | 16분 | **12분** |
| 원인 위치 | `const_fold.rs:200`, `unroll.rs:387` → SCCP 경유 | 같은 두 프레임, 독립적으로 |
| 재현기 | 트립 1 루프, 변형 파일 14개, *런타임* 트립 수의 부분 언롤도 ICE | 트립 8 루프 + 누산기; `i << 1u64`는 통과, `i << 1u32`는 ICE — 맨 `1`은 `i32` |
| 스펙 전제 정정 | `#[unroll]` 아래 `for`는 E0658이 아님(경고만) | 같음 |
| 리드 재검증 | 변형 14개 전부 재빌드, 매 행 뒤 기준선 재ICE 확인 | 기준선 + 수정 |
| 채택 | **예** (트리 + 초안), muse의 `1u64/1u32` 행을 초안에 추가 | |
| 토큰(출력, 추론 포함 / 턴) | 129k / 150 | **34k + 추론 13k / 88** |

같은 컴파일러 버그를 받은 두 팔이 서로를 보지 않고 같은 두 소스 줄에 도달했습니다 — 원인이 하나일 때 원인 규명은 그렇게 보입니다. CPU 커널에서는 둘이 같은 비율로 게이트를 넘었고, 결정은 게이트가 덮지 않은 행(M=8, SMT, 작은 형상)에서 나왔습니다. 두 팔에서 포화하는 게이트는 게이트일 뿐 비교가 아닙니다. 네 라운드 열 팔에 걸쳐 GLM은 모든 과제에서 muse의 2.7~6배 출력 토큰을 썼고, 한 팔을 고른 네 번의 채택 중 세 번을 이겼습니다. 이 두 사실 중 어느 쪽이 배정을 정해야 하는지 시리즈는 말하지 않습니다. 다음 라운드가 추측하지 않도록 표를 둡니다.
